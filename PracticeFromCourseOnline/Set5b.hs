...
cull v (Node x l r)
  | x == v    = Empty
  | otherwise = Node x (cull v l) (cull v r)
...
set (s:ss) v (Node x l r) =
  case s of
    StepL -> Node x (set ss v l) r
    StepR -> Node x l (set ss v r)
...
search target (Node x l r)
  | target == x = Just []
  | otherwise =
      case search target l of
        Just path -> Just (StepL : path)
        Nothing ->
          case search target r of
