...
sums i
    | i == 0        = []
    | i == 1        = [1]
    | otherwise     = sums (i-1) ++ [sum (sums (i-1)) + i]
...
--decremented order
isListSortedInDecrementalOrder :: [Int] -> Bool
isListSortedInDecrementalOrder [] = True
isListSortedInDecrementalOrder [_] = True
isListSortedInDecrementalOrder (x:y:xs)
    | x >= y    = isListSortedInDecrementalOrder(x:xs)
    | otherwise = False
