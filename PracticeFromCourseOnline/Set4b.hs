...
--Define helper functions so that:
--returns the reversed list.

myReverse :: [a] -> [a]
myReverse xs = foldr revHelper revStart xs

revStart:: [a]
revStart = []

revHelper :: a -> [a] -> [a]
revHelper x acc = acc ++ [x]


myFilter :: (a -> Bool) -> [a] -> [a]
myFilter p xs = foldr (filterHelper p) filterStart xs

filterStart :: [a]
filterStart = []

filterHelper :: (a -> Bool) -> a -> [a] -> [a]
filterHelper cmp x acc
    | cmp x     = x: acc
    | otherwise = acc


myPartition :: (a -> Bool) -> [a] -> ([a],[a])
myPartition p xs = foldr (partHelper p) partStart xs

partStart :: ([a],[a])
partStart = ([], [])

partHelper :: (a -> Bool) -> a -> ([a],[a]) -> ([a],[a])
partHelper p x (ts, fs)
  | p x       = (x:ts, fs)
  | otherwise = (ts, x:fs)


myLength :: [a] -> Int
myLength xs = foldr lengthHelper lengthStart xs

lengthStart :: Int
lengthStart = 0

lengthHelper :: a -> Int -> Int
lengthHelper _ acc = acc + 1
